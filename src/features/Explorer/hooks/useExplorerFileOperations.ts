...
export function useExplorerFileOperations({
  ...
  const revealInFileManager = async (path: string) => {
    try {
      ...
      toast.error(`Failed to reveal in file manager: ${error}`);
    }
  };

  const initGitRepo = async (path: string) => {
    try {
      const result = await invoke<boolean>("init_git_repo", { workingDir: path });
      if (result) {
      ...
      toast.error(`Failed to initialize git repository: ${error}`);
    }
  };
  return {
    handleOpenFolder,
